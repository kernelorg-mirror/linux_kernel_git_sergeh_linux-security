...
static int cgroup_show_path(struct seq_file *sf, struct kernfs_root *kf_root)
{
	int len = 0, ret = 0;
	char *buf = NULL;
	...
	len = kernfs_path_from_node(kf_root->kn, ns_cgroup->kn, NULL, 0);
	if (len > 0)
		buf = kmalloc(len + 1, GFP_ATOMIC);
	if (buf)
		ret = kernfs_path_from_node(kf_root->kn, ns_cgroup->kn, buf, len + 1);
